...
describe('Layout',() => {
    ...
    it('display the label provided the props',() => {
        const {queryByText} = render(<Input label="Test label"/>);
        const label = queryByText('Test label');
        expect(label).toBeInTheDocument();
    });
    it('does not display the label when no label provided as props',() => {
        const {container} = render(<Input/>);
    ...
    it('has password type for input password type provided as props',() => {
        const {container} = render(<Input type="password"/>);
        const input = container.querySelector('input');
        expect(input.type).toBe('password');
    });
    it('displays placeholder when it is provided as props',() => {
        const {container} = render(<Input placeholder="Test placholder"/>);
        const input = container.querySelector('input');
        expect(input.placeholder).toBe('Test placholder');
    });
